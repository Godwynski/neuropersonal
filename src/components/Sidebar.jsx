...
export default function Sidebar({ activeStyle, stats, theme, advancedMode, setAdvancedMode, setActiveTab }) {
  ...
  return (
    <aside className={`w-[220px] ${activeStyle.sidebarBg} border-r flex flex-col justify-between p-4 shrink-0 font-mono`}>
      <Tabs.List className="flex flex-col gap-1">
        {navItems.map(({ value, label, icon: Icon, iconClass }) => (
          <Tabs.Trigger
            key={value}
            value={value}
            className={`titlebar-no-drag w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-semibold tracking-wide transition-all border ${inactiveClass} data-[state=active]:${activeStyle.bgAccentActive}`}
          >
      ...
      <div className={`flex flex-col gap-3 border-t pt-4 text-xs ${activeStyle.border}`}>
        ...
        <div className={`flex items-center justify-between p-2 rounded-lg border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/40 border-white/5'}`}>
          <div className="flex items-center gap-2">
            ...
            <span className={`font-bold text-[10px] uppercase tracking-widest ${activeStyle.textSub}`}>Advanced</span>
          </div>
          <button
            onClick={() => {
            ...
            className={`w-8 h-4 rounded-full transition-colors relative ${advancedMode ? 'bg-green-500' : 'bg-slate-600'}`}
          >
            <span 
        ...
        <div>
          <span className={`font-bold block px-1 mb-1.5 text-[10px] uppercase tracking-widest ${activeStyle.textSub}`}>Active Theme</span>
        ...
        {stats.cpuLoad > 0 && (
          <div className={`px-2 py-2 rounded-lg border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/40 border-white/5'}`}>
            <span className={`text-[10px] uppercase tracking-widest font-bold block mb-1 ${activeStyle.textSub}`}>CPU Load</span>
            ...
            <span className={`text-[10px] mt-1 block font-bold ${activeStyle.textBody}`}>{stats.cpuLoad}% used</span>
          </div>
        )}
      </div>
    </aside>
