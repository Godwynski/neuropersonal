 ...
 * Uses Buffer.from with 'utf16le' encoding to correctly handle surrogate pairs (#40).
 */
...
/** Resolve and cache the primary GPU name + vendor (#39). */
async function getCachedGpuName() {
  if (cachedGpuName) return cachedGpuName;
  try {
    cachedGpuName = await execAsync('powershell -NoProfile -Command "(Get-CimInstance Win32_VideoController | Select-Object -First 1).Name"');
...
app.on('before-quit', () => {
  ...
  try {
    if (timerResolutionProcess && timerResolutionProcess.pid) {
      try { process.kill(timerResolutionProcess.pid); } catch (e) {}
...
// Extended sanitizer for value names that may contain file paths (#6)
function sanitizeRegistryValueNameOrPath(valueName) {
  if (typeof valueName !== 'string') return null;
  // Allow alphanumeric, spaces, dots, underscores, hyphens, backslashes, colons, parens
  if (!/^[\w\s.\-\\/:()]+$/.test(valueName)) return null;
...
// ─────────────────────────────────────────────────────────────────────────────
// Async Registry Helpers (#1: all converted from execSync to execAsync)
// ─────────────────────────────────────────────────────────────────────────────
...
async function getRegistryValue(keyPath, valueName) {
  ...
  try {
    return await execAsync(`powershell -NoProfile -Command "(Get-ItemProperty -Path '${safeKey}' -Name '${safeVal}' -ErrorAction SilentlyContinue).${safeVal}"`);
  } catch (err) {
    return '';
  }
}
  ...
  try {
    ...
    if (!exists && val !== '') {
      backups.push({
        ...
        valueName: safeVal,
        value: val,
        // #14: Store the registry type so restore uses the correct type
...
async function setRegistryValue(keyPath, valueName, value, type = 'DWord') {
  ...
  try {
    await execAsync(`powershell -NoProfile -Command "if (-not (Test-Path '${safeKey}')) { New-Item -Path '${safeKey}' -Force | Out-Null }"`);
  } catch(e) {}
  await execAsync(`powershell -NoProfile -Command "Set-ItemProperty -Path '${safeKey}' -Name '${safeVal}' -Value ${value} -Type ${type} -Force -ErrorAction Stop"`);
}
...
async function removeRegistryValue(keyPath, valueName) {
  ...
  if (!safeKey || !safeVal) throw new Error(`Unsafe registry path or value name rejected: ${keyPath} / ${valueName}`);
  await backupRegistryValueBeforeChange(safeKey, safeVal);
  await execAsync(`powershell -NoProfile -Command "Remove-ItemProperty -Path '${safeKey}' -Name '${safeVal}' -Force -ErrorAction SilentlyContinue"`);
}

// #6: Variant for registry value names that are file paths (disableFullscreenOpt)
async function setRegistryPathValue(keyPath, pathValueName, value, type = 'String') {
  ...
  // Use encoded command to avoid shell quoting issues with backslashes
...
async function getActiveGpuDevicePath() {
  try {
    ...
    const findCmd = `powershell -NoProfile -Command "Get-ChildItem '${classPath}' -ErrorAction SilentlyContinue | Where-Object { (Get-ItemProperty $_.PSPath -ErrorAction SilentlyContinue).DriverDesc -eq '${gpuName.replace(/'/g, "''")}' } | Select-Object -ExpandProperty PSChildName"`;
    const childName = await execAsync(findCmd);
    if (childName) {
...
ipcMain.handle('get-system-stats', async () => {
  ...
  try {
    const output = await execAsync('tasklist /FI "IMAGENAME eq VALORANT-Win64-Shipping.exe" /NH');
...
ipcMain.handle('set-dashboard-tweak', async (event, { tweakName, active, extraArgs }) => {
  try {
    if (tweakName === 'hags') {
      ...
      const val = active ? '0' : '1';
      await execAsync(`powercfg /SETACVALUEINDEX SCHEME_CURRENT 2a737441-1930-4402-8d77-b2bebba308a3 48e6b7a6-50f5-4782-a5d4-53bb8f07e226 ${val}`);
      await execAsync(`powercfg /SETDCVALUEINDEX SCHEME_CURRENT 2a737441-1930-4402-8d77-b2bebba308a3 48e6b7a6-50f5-4782-a5d4-53bb8f07e226 ${val}`);
      await execAsync('powercfg /setactive SCHEME_CURRENT');
    }
    else if (tweakName === 'disableCoreParking') {
      await execAsync("powercfg -attributes 54533251-82be-4824-96c1-47b60b740d00 0cc5b647-c1df-4637-891a-dec35c318583 -ATTRIB_HIDE");
      const val = active ? '100' : '5';
      await execAsync(`powercfg -setacvalueindex scheme_current sub_processor CPMinCores ${val}`);
      await execAsync(`powercfg -setdcvalueindex scheme_current sub_processor CPMinCores ${val}`);
      await execAsync('powercfg /setactive SCHEME_CURRENT');
    }
      ...
      if (active) {
        ...
        try { await execAsync('bcdedit /deletevalue disabledynamictick'); } catch (e) {}
      }
    }
      ...
      await execAsync(`powershell -NoProfile -Command "Get-NetAdapterAdvancedProperty -DisplayName '*Energy*', '*Power Saving*', '*Green*', '*EEE*' -ErrorAction SilentlyContinue | ForEach-Object { Set-NetAdapterAdvancedProperty -Name $_.Name -DisplayName $_.DisplayName -DisplayValue '${val}' -ErrorAction SilentlyContinue }"`);
    }
      ...
      if (gamePath) {
        ...
        if (active) {
          ...
          try {
            await execAsync(`powershell -NoProfile -Command "Remove-Item -Path 'HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\${exeName}\\PerfOptions' -Force -ErrorAction SilentlyContinue"`);
          } catch (e) {}
        }
      }
    }
      ...
      if (gameName && allowedGameNames.includes(gameName)) {
        await execAsync(`powershell -NoProfile -Command "Get-Process -Name '${gameName}' -ErrorAction SilentlyContinue | ForEach-Object { $_.PriorityClass = 'High' }"`);
      }
    }
      ...
      if (serviceName && allowedServiceNames.has(serviceName)) {
        ...
        await execAsync(`powershell -NoProfile -Command "Set-Service -Name '${serviceName}' -StartupType ${startType} -ErrorAction SilentlyContinue; ${action}-Service -Name '${serviceName}' -ErrorAction SilentlyContinue"`);
      } else {
...
ipcMain.handle('run-cache-cleaner', async (event, type) => {
  try {
    if (type === 'scan') {
      ...
      await execAsync("powershell -NoProfile -Command \"Remove-Item -Path '$env:TEMP\\*' -Recurse -Force -ErrorAction SilentlyContinue\"");
    }
    else if (type === 'purgeValLogs') {
      await execAsync("powershell -NoProfile -Command \"if (Test-Path '$env:LOCALAPPDATA\\VALORANT\\Saved\\Logs') { Remove-Item -Path '$env:LOCALAPPDATA\\VALORANT\\Saved\\Logs\\*' -Recurse -Force -ErrorAction SilentlyContinue }\"");
    }
    else if (type === 'purgeShader') {
      await execAsync("powershell -NoProfile -Command \"Remove-Item -Path '$env:LOCALAPPDATA\\NVIDIA\\DXCache\\*', '$env:LOCALAPPDATA\\NVIDIA\\GLCache\\*', '$env:LOCALAPPDATA\\AMD\\DxCache\\*', '$env:LOCALAPPDATA\\D3DSCache\\*' -Recurse -Force -ErrorAction SilentlyContinue\"");
    }
    return { success: true };
  } catch (err) {
    return { success: false, error: err.message };
  ...
  try {
    ...
    if (fs.existsSync(p)) {
      ...
      if (backupIndex < backups.length) {
        ...
        await execAsync(`powershell -NoProfile -Command "if (-not (Test-Path '${safeKey}')) { New-Item -Path '${safeKey}' -Force | Out-Null }"`).catch(() => {});
        ...
        await execAsync(`powershell -NoProfile -Command "Set-ItemProperty -Path '${safeKey}' -Name '${safeVal}' -Value ${valParam} -Type ${typeParam} -Force"`);
...
ipcMain.handle('check-vbs-status', async () => {
  try {
    const vbsRes = await execAsync("powershell -NoProfile -Command \"(Get-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard' -Name 'EnableVirtualizationBasedSecurity' -ErrorAction SilentlyContinue).EnableVirtualizationBasedSecurity\"").catch(() => '');
    const miRes = await execAsync("powershell -NoProfile -Command \"(Get-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard\\Scenarios\\HypervisorEnforcedCodeIntegrity' -Name 'Enabled' -ErrorAction SilentlyContinue).Enabled\"").catch(() => '');
    
    let vmPlatform = 'unknown';
    let hypervisorPlatform = 'unknown';
    try {
      const vmRes = await execAsync("powershell -NoProfile -Command \"(Get-WindowsOptionalFeature -Online -FeatureName VirtualMachinePlatform -ErrorAction SilentlyContinue).State\"");
      vmPlatform = vmRes.toLowerCase() === 'enabled' ? 'enabled' : 'disabled';
    } catch (e) {}
    try {
      const hvRes = await execAsync("powershell -NoProfile -Command \"(Get-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform -ErrorAction SilentlyContinue).State\"");
...
ipcMain.handle('toggle-vbs', async (event, enable) => {
  try {
    ...
    await execAsync(`powershell -NoProfile -Command "Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard' -Name 'EnableVirtualizationBasedSecurity' -Value ${val} -Type DWord -Force -ErrorAction Stop"`);
    await execAsync(`powershell -NoProfile -Command "if (-not (Test-Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard\\Scenarios\\HypervisorEnforcedCodeIntegrity')) { New-Item -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard\\Scenarios\\HypervisorEnforcedCodeIntegrity' -Force | Out-Null }; Set-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard\\Scenarios\\HypervisorEnforcedCodeIntegrity' -Name 'Enabled' -Value ${val} -Type DWord -Force"`);

    if (enable) {
      try { await execAsync('dism /online /Enable-Feature /FeatureName:VirtualMachinePlatform /NoRestart /Quiet'); } catch (e) {}
      try { await execAsync('dism /online /Enable-Feature /FeatureName:HypervisorPlatform /NoRestart /Quiet'); } catch (e) {}
    } else {
      try { await execAsync('dism /online /Disable-Feature /FeatureName:VirtualMachinePlatform /NoRestart /Quiet'); } catch (e) {}
      try { await execAsync('dism /online /Disable-Feature /FeatureName:HypervisorPlatform /NoRestart /Quiet'); } catch (e) {}
    }
...
ipcMain.handle('check-hpet-status', async () => {
  try {
    const res = await execAsync("powershell -NoProfile -Command \"bcdedit /enum {current} | Select-String -Pattern 'useplatformclock'\"");
...
ipcMain.handle('toggle-hpet', async (event, disable) => {
  try {
    if (disable) {
      await execAsync('bcdedit /set useplatformclock false');
    } else {
      try { await execAsync('bcdedit /deletevalue useplatformclock'); } catch (e) {}
    }
    return { success: true, rebootRequired: true };
  } catch (err) {
    return { success: false, error: err.message };
...
ipcMain.handle('toggle-amd-mpo', async (event, disable) => {
  try {
    if (disable) {
      await execAsync("powershell -NoProfile -Command \"Set-ItemProperty -Path 'HKLM:\\SOFTWARE\\Microsoft\\Windows\\Dwm' -Name 'OverlayTestMode' -Value 5 -Type DWord -Force\"");
    } else {
      await execAsync("powershell -NoProfile -Command \"Remove-ItemProperty -Path 'HKLM:\\SOFTWARE\\Microsoft\\Windows\\Dwm' -Name 'OverlayTestMode' -Force -ErrorAction SilentlyContinue\"");
    }
    return { success: true };
  } catch (err) {
    return { success: false, error: err.message };
...
ipcMain.handle('check-gpu-driver-profile', async () => {
  try {
    ...
    if (vendor === 'nvidia') {
      ...
      try {
        const pmRes = await execAsync(`powershell -NoProfile -Command "(Get-ItemProperty -Path '${tweakPath}' -Name 'PowerMizerEnable' -ErrorAction SilentlyContinue).PowerMizerEnable"`);
        const pmLevel = await execAsync(`powershell -NoProfile -Command "(Get-ItemProperty -Path '${tweakPath}' -Name 'PowerMizerLevel' -ErrorAction SilentlyContinue).PowerMizerLevel"`);
        result.powerMaxPerformance = pmRes === '1' && pmLevel === '1';
      } catch (e) { result.powerMaxPerformance = false; }

      try {
        const llRes = await execAsync(`powershell -NoProfile -Command "(Get-ItemProperty -Path '${tweakPath}' -Name 'LowLatencyMode' -ErrorAction SilentlyContinue).LowLatencyMode"`);
        result.lowLatencyUltra = llRes === '3';
      } catch (e) { result.lowLatencyUltra = false; }

      try {
        const toRes = await execAsync(`powershell -NoProfile -Command "(Get-ItemProperty -Path '${tweakPath}' -Name 'ThreadedOptimization' -ErrorAction SilentlyContinue).ThreadedOptimization"`);
      ...
      try {
        const alRes = await execAsync(`powershell -NoProfile -Command "$k = ${amdFilter}; if ($k) { (Get-ItemProperty $k.PSPath -Name 'KMD_EnableAntiLag' -ErrorAction SilentlyContinue).KMD_EnableAntiLag } else { '' }"`);
        result.antiLagEnabled = alRes === '1';
      } catch (e) { result.antiLagEnabled = false; }

      try {
        const tfRes = await execAsync(`powershell -NoProfile -Command "$k = ${amdFilter}; if ($k) { (Get-ItemProperty $k.PSPath -Name 'TextureFilterQuality' -ErrorAction SilentlyContinue).TextureFilterQuality } else { '' }"`);
        result.textureFilterPerformance = tfRes === '0';
      } catch (e) { result.textureFilterPerformance = false; }

      try {
        const chillRes = await execAsync(`powershell -NoProfile -Command "$k = ${amdFilter}; if ($k) { (Get-ItemProperty $k.PSPath -Name 'KMD_EnableRadeonChill' -ErrorAction SilentlyContinue).KMD_EnableRadeonChill } else { '1' }"`);
        result.radeonChillDisabled = chillRes === '0';
      } catch (e) { result.radeonChillDisabled = false; }

      try {
        const boostRes = await execAsync(`powershell -NoProfile -Command "$k = ${amdFilter}; if ($k) { (Get-ItemProperty $k.PSPath -Name 'KMD_EnableRadeonBoost' -ErrorAction SilentlyContinue).KMD_EnableRadeonBoost } else { '1' }"`);
  ...
  try {
    if (vendor === 'nvidia') {
      ...
      if (profile === 'performance') {
        await execAsync(`powershell -NoProfile -Command "${ensurePath} Set-ItemProperty -Path '${tweakPath}' -Name 'PowerMizerEnable' -Value 1 -Type DWord -Force; Set-ItemProperty -Path '${tweakPath}' -Name 'PowerMizerLevel' -Value 1 -Type DWord -Force; Set-ItemProperty -Path '${tweakPath}' -Name 'LowLatencyMode' -Value 3 -Type DWord -Force; Set-ItemProperty -Path '${tweakPath}' -Name 'ThreadedOptimization' -Value 1 -Type DWord -Force"`);
      } else {
        await execAsync(`powershell -NoProfile -Command "${ensurePath} Remove-ItemProperty -Path '${tweakPath}' -Name 'PowerMizerEnable' -Force -ErrorAction SilentlyContinue; Remove-ItemProperty -Path '${tweakPath}' -Name 'PowerMizerLevel' -Force -ErrorAction SilentlyContinue; Remove-ItemProperty -Path '${tweakPath}' -Name 'LowLatencyMode' -Force -ErrorAction SilentlyContinue; Remove-ItemProperty -Path '${tweakPath}' -Name 'ThreadedOptimization' -Force -ErrorAction SilentlyContinue"`);
      }
    } else if (vendor === 'amd') {
      const amdClassPath = 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}';
...
ipcMain.handle('check-hardware-bottlenecks', async () => {
  try {
    let ramModules = [];
    try {
      const ramRes = await execAsync("powershell -NoProfile -Command \"Get-CimInstance Win32_PhysicalMemory | Select-Object Speed, ConfiguredClockSpeed, Capacity, Manufacturer | ConvertTo-Json\"");
    ...
    try {
      const rebarRes = await execAsync("powershell -NoProfile -Command \"$keys = Get-ChildItem 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}' -ErrorAction SilentlyContinue; foreach ($k in $keys) { $val = (Get-ItemProperty $k.PSPath -Name 'KMD_ReBarEnable' -ErrorAction SilentlyContinue).KMD_ReBarEnable; if ($val -eq 1) { echo 'enabled'; break } }\"");
      if (!rebarRes.includes('enabled')) {
        const above4g = await execAsync("powershell -NoProfile -Command \"(Get-ItemProperty -Path 'HKLM:\\SYSTEM\\CurrentControlSet\\Control\\Class\\{4d36e968-e325-11ce-bfc1-08002be10318}\\0000' -Name 'KMD_EnableReBarSupport' -ErrorAction SilentlyContinue).KMD_EnableReBarSupport\"").catch(() => '');
